...
if (numelems(var) = 1)  then simplify(LinearAlgebra[Norm](diff(r(var[1]),var[1]),2)) assuming u::real

elif (numelems(var) = 2 and numelems(r(vop(var))) = 2) then 
...
rot:=proc(V::{procedure, Vector}) uses VectorCalculus;BasisFormat(false);
if (type(V,procedure)) then Curl(Student[VectorCalculus][VectorField](V(x,y,z)));
else Curl(Student[VectorCalculus][VectorField](V));
end if;end proc:

div:= proc(V::{procedure, Vector});
if (type(V,procedure)) then unapply(VectorCalculus[Divergence](Student[VectorCalculus][VectorField](V(x,y,z))),[x,y,z]);
...
flux:= proc(V::{procedure},r::{procedure},integrateRange::{list});
local var,i;
var := [op(1,eval(r))]:
...
tangielt:= proc(V::{procedure},r::{procedure},integrateRange::{range})
local var,i;
var := [op(1,eval(r))]:
int(prik(V(vop(r(var[1]))),diff(r(var[1]),var[1])),var[1]=integrateRange);
end proc:

stokes:= proc(V::{procedure},r::{procedure},integrateRange::{list});
local var,i;
flux(unapply(rot(V),[x,y,z]),r,integrateRange);
end proc:

flowkurve:= proc(V::{procedure},punkt::{list} := [1]);
local i, var, funk,løs;
var := [op(1,eval(V))]:
funk := [seq(var[i](t),i=1..3)]:
løs := dsolve([seq(diff(funk[i],t)=diff(V(vop(var)),var[1])[i]*funk[1]+diff(V(vop(var)),var[2])[i]*funk[2]+diff(V(vop(var)),var[3])[i]*funk[3],i=1..3), if numelems(punkt) > 1 then seq(var[i](punkt[1])=punkt[i+1],i=1..3) else end if]):
...
local var,i;
var := [op(1,eval(f))]:
unapply(mtaylor(f(vop(var)),if numelems(var) = 1 then var[1] = punkt else [seq(var[i]=punkt[i],i=1..numelems(var))] end if, grad+1),[seq(var[i],i=1..numelems(var))]);
end proc:
